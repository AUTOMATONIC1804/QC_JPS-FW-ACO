...
fw_jps_runner.py
-----------------------------
Floyd–Warshall (FW) for JPS-generated route (GeoTIFF-based).

- JPS route GeoJSON: path line (EPSG:3857)
...
def load_jps_route(route_geojson):
    ...
    if gdf.crs is None:
        warnings.warn("JPS route missing CRS; assuming EPSG:3857.")
...
def run_fw_jps_raster(
    ...
    print("🚆 Running FW (JPS with real road lines from GeoTIFF)")
    ...
    # 1. Load route + buffer
    route_line = load_jps_route(route_geojson)
