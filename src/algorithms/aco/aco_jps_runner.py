...
6) Export stations, path, buffers, all-nodes CSV, convergence logs
"""
...
def run_aco_jps(
    ...
    # 13) Export route line
    coords = [points_feas.iloc[i].geometry for i in best_route if not points_feas.iloc[i].geometry.is_empty]
    if len(coords) > 1:
        route_line = LineString(coords)
        gpd.GeoDataFrame({"role": ["path"], "geometry": [route_line]}, crs="EPSG:3857") \
           .to_crs("EPSG:4326") \
           .to_file(outdir / "aco_jps_path.geojson", driver="GeoJSON")
